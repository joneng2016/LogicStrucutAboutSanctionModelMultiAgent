...
\begin{enumerate}
	...
	\item $isPreRequisite(gp_i,g_j)$ - Os objetivos $g$ pertencentes ao grupo $gp_i$ devem ser concluído para que haja condição de executar $g_j$.	
...
Uma violação de condição tem como consequências a ocorrência do risco (sobre o agente que cometeu a violação) relacionado a condição que não estava presente. Esse risco pode apresentar diferentes graus de fatalidade.
...
\begin{eqnarray}\label{rel13}\nonumber
	hasRelation(g_i,rg_n) \wedge (r_k \subset rg_n) \wedge hasRisk(r_k,risk_j,f_m) \wedge tryReach(ag_m,g_i) \nonumber \\ 
