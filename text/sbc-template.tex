...
Esse estudo está interessado em sistemas multiagente onde os agentes trabalham de forma colaborativa na tentativa de alcançar um propósito em comum, que são capazes de acatar normas, que podem cometer violações, que são submetidos a sanções, que interagem com outros agentes, que interagem com artefatos em sua volta, que possuem papeis claros, que percorrem objetivos claramente definidos, que são submetidos a um dado risco e que, na ocorrência de um acidente, se submetem a consequências ruins. 

Um agente, nesse estudo, possui competências e autonomia necessária com a finalidade de alcançar os objetivos. Um agente pode ser reativo (responde a estimulos do ambiente) ou cognitivo (apresenta estados internos, possui capaciadade de realizar raciocínios). Contudo não é de interesse dos pesquisadores deste estudo definir um modelo delineado apenas para uma categoria específica de agentes.

O conceito de sistema multiagente adotado neste estudo consiste em uma série de regras que definem como os agentes se comportam dentro de uma estrutura única.

Normas são instruções claras do que um agente deve fazer. Uma violação, portanto, acontece quando um agente realiza a desobediência de uma norma. Se o agente desobediente sofrer uma punição pelo que fez, então esse agente foi submetido a uma sanção. 	
...
O modelo proposto neste estudo tem a finalidade representar atividades onde um grupo de pessoas devem atuar de forma colaborativa para resolver um dado problema. Essa situação pode ser dividida em etapas menores conhecidas como objetivos. As pessoas podem se relacionar entre si assim como também podem se relacionar com os artefatos (ferramentas, equipamentos de proteção individual, estruturas, equipamentos sujeitos a manutenção) dispostos ao redor. Os artefatos também podem se relacionar com outros artefatos (ex. chave de fenda é um artefato, parafuso é outro artefato, chave de fenda sendo usada para desparafusar o parafuso é uma relação entre esses dois artefatos).

Para que um dado objetivo seja concluído de forma eficaz, é necessário que os profissionais envolvidos tenham acesso a certos artefatos. Não apenas isso, mas os profissionais devem ter a capacidade de estabelecer as relações necessárias para que o objetivo seja alcançado.
...
No que diz respeito as consequências dos erros, os pesquisadores se limitaram a duas situações: 1 - consequências negativas imediatas que acontecem sobre o indivíduo errante, 2 - a consequência ruim que se manifesta em objetivos diferentes daquele em que o erro foi cometido e sobre o mesmo ou outro indivíduo. Em ambos os casos essas consequências são efeitos físicos negativos que alguém passa a sofrer. A intensidade dessas consequências variam desde uma leve lesão a morte.
