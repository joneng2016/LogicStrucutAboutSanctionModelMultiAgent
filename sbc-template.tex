...
\begin{eqnarray}\label{rel13}\nonumber
	happensBadEvent(g_n,r_k) \wedge hasRisk(r_k,risk_j,f_m) \wedge tryReach(ag_m,g_n) \nonumber \\ 
	\to consequenceOfBadEvent(g_n,ag_m,risk_j,f_m)
\end{eqnarray}
...
Se uma gente é obrigado a fazer algo, então esse agente deve ter a permissão para isso. Caso contrário, por um impedimento sistemico, um agente nunca conseguirá cumprir com aquilo a que ele é obrigado a fazer. Isso é definido pela regra \ref{rel1} uma obrigação implica necessariamente em uma permissão. Outros modelos adotam uma abordagem similar a essa \cite{mosieframework}. 
...
Supondo não ser possível afirmar sobre a presenção de uma entidade necessária para conclusão de um determinado objetivo. Assim sendo, também não é possível realizar qualquer afirmação sobre uma violação deste gênero. 
...
g2 &  g1 & Medir a corrente de fulga de ferramentas isolantes \\ \hline
...
g8 &  g7 & Passar o bastao universal por dentro do olhal do colar \\ \hline
g9 &  g8 & Pender carretilha no bastão Universal. \\ \hline
g10 &  g9 & Amarrar o bastão universal na parte superior da estrutra com a corda; \\ \hline
...
O \textit{V3S} possuem representações para defini as entidades, objetos e ações assim como o modeo presente neste estudo. Contudo, existe diferenças na estrutra ontologica de ambo os modelos, pois o \textit{V3S} define tanto objetos e açõe como entidades, enquanto que o modelo proposto neste estudo apenas objetos são entidades. O \textit{V3S} não caracteriza os agentes dentro de sua estrutura ontologica, enquanto que o modelo deste estudo faz isso definindo os agentes como entidades que se relacionam com outras entidades. 
